      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            <img src={logo} alt="Farms Fresh Food" className="h-16 w-auto group-hover:scale-105 transition-transform duration-300" />
            <div className="hidden sm:block">
              <h1 className={`font-serif text-lg font-bold leading-tight ${isScrolled ? "text-foreground" : "text-cream"}`}>
                FARMS FRESH FOOD
              </h1>
              <span className={`text-xs tracking-widest ${isScrolled ? "text-gold" : "text-gold-light"}`}>EVENTS, KITCHEN SERVICES AND FOOD CATERING</span>
            </div>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              {socialLinks.map(social => <a key={social.label} href={social.href} target="_blank" rel="noopener noreferrer" className={`p-2 rounded-full transition-all duration-300 ${isScrolled ? "text-foreground/70 hover:text-gold hover:bg-gold/10" : "text-cream/70 hover:text-cream hover:bg-white/10"}`} aria-label={social.label}>
